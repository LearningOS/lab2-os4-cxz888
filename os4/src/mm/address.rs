...
impl PhysPageNum {
    ...
    pub fn as_pte_slice(&self) -> &'static mut [PageTableEntry; PTE_PER_PAGE] {
        let pa = self.page_start();
        unsafe {
            (pa.0 as *mut [PageTableEntry; PTE_PER_PAGE])
                .as_mut()
                .unwrap()
        }
    }
    pub fn as_page_bytes(&self) -> &'static [u8; PAGE_SIZE] {
        let pa = self.page_start();
        unsafe { (pa.0 as *const [u8; PAGE_SIZE]).as_ref().unwrap() }
    }
    // pub fn as_page_bytes_mut(&self) -> &'static mut [u8; PAGE_SIZE] {
    //     let pa = self.page_start();
    //     unsafe { (pa.0 as *mut [u8; PAGE_SIZE]).as_mut().unwrap() }
    // }
    ...
    pub fn as_mut_at<T>(&self, offset: usize) -> &'static mut T {
        let pa = self.page_start();
